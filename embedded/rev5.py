...
def align_sorter_to_bin(denomination):
    ...
    for _ in range(max_attempts):
        ...
        if error < 0:
            move_stepper(SORTER_STEP, SORTER_DIR, direction=False, steps=200, delay=0.001)
        else:
            move_stepper(SORTER_STEP, SORTER_DIR, direction=True, steps=200, delay=0.001)
...
def process_bill(baseline):
    ...
    move_stepper(ACCEPTOR_STEP, ACCEPTOR_DIR, direction=True, steps=250, delay=0.001)
...
try:
	...
	baseline = get_average_distance(acceptor_sensor, samples=10)
	print(f"Baseline (no bill): {baseline} cm")
	while True:
		process_bill(baseline)
		time.sleep(2)  # Brief delay before accepting next bill
